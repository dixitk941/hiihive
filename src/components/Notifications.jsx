...
const NotificationsPage = () => {
  ...
  return (
    <div className="notifications-page-container bg-white text-gray-800 min-h-screen p-4 sm:p-5 md:p-6">
      {/* Header without top margin */}
      <header className="header bg-gray-900 text-white py-4 px-6 rounded-lg shadow-md">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-semibold tracking-tight">Notifications</h1>
      </header>

      {/* Content Container without top margin */}
      <div className="content-container mt-4 sm:mt-6 md:mt-8">
        {notifications.length === 0 ? (
          <p className="text-center text-lg sm:text-xl text-gray-500">You have no notifications.</p>
        ) : (
            ...
            {notifications.map((notification) => (
              <li
                ...
                className={`notification-item p-4 sm:p-5 md:p-6 rounded-lg shadow-md transform transition-all duration-200 ease-in-out hover:scale-105 ${
                  notification.seen ? 'bg-gray-100' : 'bg-white border border-gray-300'
                }`}
              >
                <p className="font-medium text-base sm:text-lg text-black">{notification.message}</p>
